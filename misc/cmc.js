    ...
    return cmc.getIdMap({symbol: coinCode}).then(results =>{
        ...
        for (let i =0; i< data.length; i++){
            ...
            if (platform && platform.name == "Ethereum"){
                ...
                htmlString += "<a href=\"" + uniswapurl + "\" target=\"_blank\"> Swap " + item.name + " on Uniswap, Contract "+token_address+"</a>\n"
            }
            if (platform && platform.name == "Binance Smart Chain"){
                ...
                htmlString += "<a href=\"" + pancakeswapurl + "\" target=\"_blank\"> Swap " + item.name + " on Pancake contract " +token_address+ "</a>\n"
            }
